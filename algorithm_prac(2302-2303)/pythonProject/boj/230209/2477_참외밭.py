...
for _ in range(6):
    directions, lengths = map(int,input().split())
    direc.append(directions)
    leng.append(lengths)
     # 큰 박스의 길이, 작은 박스의 길이를 담을 배열
...
for i in range(1, 5):
    # print(direc.count(i))
    if direc.count(i) == 1: # direction이 한번만 존재한다 == 큰 박스의 변

        max_lengths.append(leng[direc.index(i)]) # 큰박스의 변 길이 저장
        print(max_lengths)
        temp = direc.index(i) + 3 # 큰 박스 + 3 == 작은 박스의 변
        print(direc.index(i))
        # print(temp)
        if temp >= 6:
